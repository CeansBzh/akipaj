<section x-data="app()">
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">Assurez-vous d'utiliser un mot de passe long et complexe pour protéger
            votre compte.</p>
    </header>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div class="relative">
            ...
            <x-text-input id="current_password" name="current_password" ::type="showOldPasswordField ? 'password' : 'text'" class="mt-1 block w-full"
                autocomplete="current-password" />
            <x-input-error :messages="$errors->updatePassword->get('current_password')" class="mt-2" />

            <button
                class="leading-0 absolute top-8 right-2 block h-7 w-7 rounded text-center text-xl text-gray-400 transition-colors hover:text-sky-500 focus:text-sky-500 focus:outline-none focus:motion-safe:animate-pulse"
                @click.prevent="showOldPasswordField=!showOldPasswordField">
                <span x-show="showOldPasswordField">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                        fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                        stroke-linejoin="round" class="m-0">
        ...
        <div class="relative">
            ...
            <div class="-mx-1 mt-2 mb-4 flex">
                <template x-for="(v,i) in 5">
                    <div class="w-1/5 px-1">
                        <div class="h-2 rounded-xl transition-colors"
                            :class="i < passwordScore ? (passwordScore <= 2 ? 'bg-red-400' : (passwordScore <= 4 ?
                                'bg-yellow-400' : 'bg-green-500')) : 'bg-gray-200'">
                        </div>
                    </div>
                </template>
            </div>
            ...
            <button
                class="leading-0 absolute top-8 right-2 block h-7 w-7 text-center text-xl text-gray-400 transition-colors hover:text-sky-500 focus:text-sky-500 focus:outline-none focus:motion-safe:animate-pulse"
