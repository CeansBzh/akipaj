<section class="mx-auto max-w-2xl" x-data="editEvent()" x-init="init()">
    <header>
    ...
    <form method="post" action="{{ route('events.update', $event) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
        @csrf
        @method('patch')
        ...
        <div>
            ...
            <x-text-input id="location_input" name="location" type="text" class="mt-1 block w-full" :value="$event->location"
        ...
        <div class="relative" x-show="!imageRemoved">
            <img id="image_display" src="{{ $event->imagePath }}"
                alt="Image de couverture de l'événement {{ $event->name }}"
