<div x-data="{ open: false }" @load.window="open = true" x-init="setTimeout(() => open = false, 6000)">
    <div id="alert-{{ $id }}" class="flex p-4 mb-4 bg-{{ $color }}-100 text-{{ $color }}-700 rounded-lg" role="alert"
        ...
        <button type="button"
            class="ml-auto -mx-1.5 -my-1.5 bg-{{ $color }}-100 text-{{ $color }}-500 rounded-lg focus:ring-2 focus:ring-{{ $color }}-400 p-1.5 hover:bg-{{ $color }}-200 inline-flex h-8 w-8"
